//------------------------------------------------------------------------
// <copyright file="UploadNationalJetFuelInvoiceController.cs" company="AACOSTA">
...
namespace VOI.SISAC.Web.Areas.Process.Controllers
{
    ...
    /// <summary>
    /// Upload National Jet Fuel Invoice Controller
    /// </summary>
    /// <seealso cref="VOI.SISAC.Web.Controllers.BaseController" />
    public class UploadNationalJetFuelInvoiceController : BaseController
    {
        /// <summary>
        /// The logger
        /// </summary>
        ...
        /// Catalog name
        /// </summary>
        private readonly string catalogName = "Upload National Jet Fuel Invoice";
        ...
        public UploadNationalJetFuelInvoiceController(INationalJetFuelInvoiceControlBusiness nationalJetFuelInvoiceControl)
        {
        ...
        /// <param name="AirlineCodeCombobox">The airline code combo box.</param>
        /// <returns>Action result.</returns>
        [HttpPost]
        public ActionResult Upload(HttpPostedFileBase file, string AirlineCodeCombobox)
        {
            // Validates that the airline selected
            if (string.IsNullOrEmpty(AirlineCodeCombobox))
            {
                this.TempData["ErrorMessage"] = Resource.Airline + ' ' + Resource.RequiredField;
                return this.RedirectToAction("Index", "NationalJetFuelInvoice");
            }
            ...
            try
            {
                DelimitedFileEngine<NationalJetFuelInvoiceDetailFile> engine = new DelimitedFileEngine<NationalJetFuelInvoiceDetailFile>();
                engine.AfterReadRecord += EngineAfterReadRecord;
                engine.Options.IgnoreFirstLines = 1;
                engine.ErrorManager.ErrorMode = ErrorMode.SaveAndContinue;
                NationalJetFuelInvoiceDetailFile[] records;
                ...
                IList<NationalJetFuelInvoiceDetailFile> fileRecords = new List<NationalJetFuelInvoiceDetailFile>(records);

                // Creacion de DataTable para mandar por SP               
                var table = new DataTable();
                ListFileToDataTable(fileRecords, table);

                // Se envia informacion de remesas a DB
                errorResult = this.nationalJetFuelInvoiceControl.ValidateRemittanceID((DataTable)table, AirlineCodeCombobox);
                ...
                if (errorResult.Count > 0)
                {
                    ...
                    foreach (string item in errorResult)
                    {
                        if (item.Length < 22)
                        {
                            message.Add("Remittance repeated: " + item);
                        }
                        else
                        {
                            message.Add(item);
        ...
        public FileContentResult Download()
        {
            ...
            {
                byte[] data;
                string pathFile = Server.MapPath(Resource.NationalInvoiceFilePath);
                data = FileHelper.GetPlainTextFile(pathFile);
                return this.File(data, MediaTypeNames.Text.Plain, Resource.NationalInvoiceFileName);
            }
            catch (IOException)
            {
        ...
        private static void ListFileToDataTable(IList<NationalJetFuelInvoiceDetailFile> fileRecords, DataTable table)
        {
            ...
            foreach (var entity in fileRecords)
            {
                ...
                row["RemittanceID"] = entity.RemittanceID;
                row["MonthYear"] = entity.MonthYear;
                row["Period"] = entity.Period;
        ...
        /// <summary>
        /// Engine_s the after read record.
        /// </summary>
        /// <param name="engine">The engine.</param>
        /// <param name="e">The <see cref="AfterReadEventArgs{NationalJetFuelInvoiceDetailFile}"/> instance containing the event data.</param>
        private static void EngineAfterReadRecord(EngineBase engine, AfterReadEventArgs<NationalJetFuelInvoiceDetailFile> e)
        {
        ...
        /// </summary>
        /// <param name="file">The file.</param>
        /// <returns>The total of errors found.</returns>
        private IList<string> FindErrors(DelimitedFileEngine<NationalJetFuelInvoiceDetailFile> file)
        {
