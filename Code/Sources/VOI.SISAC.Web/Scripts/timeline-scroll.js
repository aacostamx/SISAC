...
function setConfig(control, selectedElement) {
    var $this = $(control);
    var style = $this.data('layout');
    var id = $this.data('id');

    // work in progress ...
    var startit = selectedElement ? selectedElement : $this.data('startat') ? $this.data('startat') : null; // Sets the initial element to show in the timeline
    //var startit = $this.data('startit') > 0 ? $this.data('startit') : 1; // Sets the initial element to show in the timeline
...
function wpex_timeline(settings) {
    if ($(settings.id_control + ' ' + settings.datesDiv).length > 0 && $(settings.id_control + ' ' + settings.issuesDiv).length > 0) {
        ...
        if (settings.startAt) {
            $('#' + settings.startAt).find('a').trigger('click');
        }
        else {
            $(settings.id_control + ' ' + settings.datesDiv + ' li').eq(0).find('a').trigger('click');
        }
